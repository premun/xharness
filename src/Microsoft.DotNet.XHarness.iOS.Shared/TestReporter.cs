...
namespace Microsoft.DotNet.XHarness.iOS.Shared {
	...
	public class TestReporter : ITestReporter {
		...
		async Task<(int pid, bool launchFailure)> GetPidFromRunLog ()
		{
			...
			if (reader.Peek () == -1)               // empty file! we definetly had a launch error in this case
		...
		async Task<bool> TcpConnectionFailed ()
		{
			...
			while ((line = await reader.ReadLineAsync ()) != null) {
				if (line.Contains ("Couldn't establish a TCP connection with any of the hostnames")) return true;
			}
			return false;
		}
		...
		async Task CollectResult (Task<ProcessExecutionResult> processExecution)
		{
			...
			if (!waitedForExit && !result.TimedOut) {
				...
				if (!await listener.CompletionTask.TimeoutAfter (timeout - timeoutWatch.Elapsed)) result.TimedOut = true;
			}
		...
		{
			if (launchResult.IsFaulted) mainLog.WriteLine ("Test launch failed: {0}", launchResult.Exception);
		...
		public async Task CollectSimulatorResult (Task<ProcessExecutionResult> processExecution)
		{
			...
			if (!Success.Value) {
				...
				if (pid > 0) await KillAppProcess (pid, cancellationTokenSource);
				else {
		...
		public async Task<(TestExecutingResult ExecutingResult, string FailureMessage)> ParseResult ()
		{
			...
			if (!Success.Value) {
				...
				if (!string.IsNullOrEmpty (crashReason)) {
					if (crashReason == "per-process-limit") result.FailureMessage = "Killed due to using too much memory (per-process-limit).";
					else {
